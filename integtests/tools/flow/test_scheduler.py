...
async def test_scheduler_lifecycle(
    mcp_context, configs, keboola_client
) -> None:
    """
    ...
    try:
        ...
        found_schedule = next(
            (s for s in schedules if s.schedule_id == created_schedule.schedule_id),
            None
        )
        ...
        found_updated_schedule = next(
            (s for s in schedules_after_update if s.schedule_id == created_schedule.schedule_id),
            None
        )
        ...
        deleted_schedule_exists = any(
            s.schedule_id == created_schedule.schedule_id
            for s in schedules_after_delete
        )
