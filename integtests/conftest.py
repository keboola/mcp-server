...
class ConfigDef:
    ...
    def file_path(self) -> Path:
        ...
        """
        return _data_dir() / 'proj' / 'configs' / self.component_id / f'{self.configuration_name}.json'
...
_CONFIG_DEFS_IN = [
    ConfigDef(
        ...
        configuration_name='test_config1',
    ),
]
...
def _create_configs(storage_client: SyncStorageClient) -> list[ConfigDef]:
    ...
    for config in _CONFIG_DEFS_IN:
        LOG.info(f'Creating config with internal ID={config.configuration_name}')
