...
async def query_data(
    ...
    query_ref: Annotated[
        ...
        Field(
            description=(
                'Opaque correlation token chosen by the agent. Pass the SAME value to validate_semantic_query so '
                'the client can pair this result with its semantic validation. Purely a passthrough; it does not '
                'affect execution and is echoed back unchanged.'
            )
        ),
    ] = None,
) -> QueryDataOutput:
