...
async def get_component_configuration_examples(
    ...
    """
    client = KeboolaClient.from_state(ctx.session.state)
    data = await client.ai_service_client.get_component_configuration_examples(component_id)
    root_examples = data.get('rootConfigurationExamples') or []
    row_examples = data.get('rowConfigurationExamples') or []
