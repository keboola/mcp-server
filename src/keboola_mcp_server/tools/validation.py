...
class RecoverableValidationError(jsonschema.ValidationError):
    ...
    def __str__(self) -> str:
        ...
        if self.validation_context:
            str_repr += f'Validation component context: {self.validation_context!s}\n'
...
class KeboolaParametersValidator:
    ...
    def check_button_type(checker: jsonschema.TypeChecker, instance: object) -> bool:
        ...
        return isinstance(instance, dict) and instance.get('type', None) == 'button'
    ...
    def sanitize_schema(schema: JsonDict) -> JsonDict:
        ...
        ) -> tuple[JsonStruct | JsonPrimitive, bool | None]:
...
def _validate_json_against_schema(
    ...
    schema: JsonDict,
    initial_message: str | None = None,
    validate_fn: ValidateFunction | None = None,
    validation_context: ValidationContext | None = None,
):
    ...
    try:
        ...
        LOG.exception(
            f'The validation schema is not valid.\n'
...
def _validate_storage_configuration(
    ...
    *,
    ...
    storage_cfg = cast(JsonDict | None, storage.get('storage', storage) if storage else {})
    ...
    if (
        component.component_id in [SNOWFLAKE_TRANSFORMATION_ID, BIGQUERY_TRANSFORMATION_ID]
        and not storage_cfg.get('input')
        and not storage_cfg.get('output')
    ):
        raise ValueError(
