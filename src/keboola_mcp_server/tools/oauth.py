...
async def create_oauth_url(
    component_id: Annotated[str, Field(description='The component ID to grant access to (e.g., "keboola.ex-google-analytics-v4").')],
    ...
    token_response = await client.storage_client.post(
        endpoint="tokens",
        data=token_data
    )
    ...
    # Generate OAuth URL
    oauth_url = f"https://external.keboola.com/oauth/index.html?token={sapi_token}&sapiUrl={storage_api_url}#/{component_id}/{config_id}"
