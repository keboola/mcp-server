...
async def create_flow(
...
) -> Annotated[FlowConfigurationResponse, Field(description='Created flow configuration')]:
    ...
    LOG.info(f'Created flow "{name}" with configuration ID "{flow_config.configuration_id}"')
    return flow_config
...
) -> Annotated[FlowConfigurationResponse, Field(description='Updated flow configuration')]:
    ...
    if flow_ids:
        ...
        for flow_id in flow_ids:
            try:
                ...
                flow = FlowSummary.from_raw_config(raw_config)
                flows.append(flow)
        ...
        flows = [FlowSummary.from_raw_config(raw_flow) for raw_flow in raw_flows]
...
async def get_flow_detail(
    ...
    flow = FlowConfigurationResponse.from_raw_config(raw_config)
...
def _ensure_task_ids(tasks: List[Dict[str, Any]]) -> List[FlowTask]:
    ...
    for task in tasks:
        ...
        if 'mode' not in task_obj:
            task_obj['mode'] = 'run'  # Schema default
