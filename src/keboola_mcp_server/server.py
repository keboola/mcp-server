...
from keboola_mcp_server.config import Config, ServerRuntimeInfo, Transport, get_deployment_storage_api_url
...
def create_server(
    ...
    *,
    ...
    # The Storage API URL of the stack this server is deployed on. It is also the URL that
    # per-request Storage API URLs are checked against, so both must be derived the same way.
    if not config.storage_api_url and (deployment_storage_api_url := get_deployment_storage_api_url()):
        config = dataclasses.replace(config, storage_api_url=deployment_storage_api_url)
