    ...
    *,
    ...
    mcp = KeboolaMcpServer(
        ...
        instructions=(
            ...
            '(kbc_at_/kbc_pat_). A session may already be pre-scoped -- e.g. the user chose specific '
            'projects at `login` time, or the token can only reach one project -- in which case data '
            'tools work immediately with no further action needed. Do NOT call "get_accessible_projects" '
            'or "set_project_scope" preemptively "just in case": just call the data tool you actually '
            'need. Only if a data tool call fails with an error asking you to confirm a project scope '
            '(this happens when the session truly has none yet): call "get_accessible_projects", show '
            'the user their projects, and ASK whether to work across ALL of them or a subset. Do not '
            'decide for them. Then call "set_project_scope" with their answer (no arguments = all '
            'projects, or the chosen project ids, optionally read_only=true). Both tools return a '
            '"scope_token" -- the server does not remember the scope between calls, so resend that value '
            'as the "scope_token" argument on every subsequent tool call in this conversation. After '
            'that, read-only tools return results per project. Never write to more than one project '
            'without explicit user confirmation — write operations target the active (first-scoped) '
            'project only. If instead the session uses a legacy project-scoped Storage API token, it is '
            'already bound to a single project: use the tools directly — "get_accessible_projects" / '
            '"set_project_scope" do not apply (they will report that no programmatic token is present). '
