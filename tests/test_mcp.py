...
class SessionStateMiddlewareTests:
    @pytest.mark.asyncio
    async def test_create_session_state_awaits_with_branch_id(self, monkeypatch):
        ...
        class DummyClient:
            ...
            def __init__(
                ...
                *,
                ...
                called['storage_api_url'] = storage_api_url
                called['storage_api_token'] = storage_api_token
                called['bearer_token'] = bearer_token
                called['headers'] = headers
                called['readonly'] = readonly

            async def with_branch_id(self, branch_id: str | None) -> 'DummyClient':
        ...
        class DummyWorkspaceManager:
            ...
            @classmethod
            async def create(cls, client: DummyClient, workspace_schema: str | None = None) -> object:
                called['workspace_client'] = client
                called['workspace_schema'] = workspace_schema
                return object()

        monkeypatch.setattr('keboola_mcp_server.mcp.KeboolaClient', DummyClient)
        monkeypatch.setattr('keboola_mcp_server.mcp.WorkspaceManager', DummyWorkspaceManager)
        ...
        state = await SessionStateMiddleware.create_session_state(config, runtime_info, readonly=True)

        assert isinstance(state[DummyClient.STATE_KEY], DummyClient)
        assert state[DummyWorkspaceManager.STATE_KEY]
        assert state['conversation_id'] == 'conv'
        assert called['branch_id'] == '123'
        assert called['workspace_schema'] == 'schema'

    @pytest.mark.asyncio
    async def test_create_session_state_requires_storage_token(self):
        config = Config(storage_api_url='https://connection.test.keboola.com', storage_token=None)
        runtime_info = ServerRuntimeInfo(transport='stdio')

        with pytest.raises(ValueError, match='Storage API token is not provided'):
            await SessionStateMiddleware.create_session_state(config, runtime_info)

    @pytest.mark.asyncio
    async def test_create_session_state_requires_storage_api_url(self):
        config = Config(storage_api_url=None, storage_token='token')
        runtime_info = ServerRuntimeInfo(transport='stdio')
