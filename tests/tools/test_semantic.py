...
from keboola_mcp_server.tools.semantic.model import SemanticDefineAction, SemanticEntityType, SemanticFilter, SemanticScope
...
async def test_semantic_get_definition_found_by_uuid(mcp_context_client: Context) -> None:
    ...
    client.metastore_client.get_object = AsyncMock(
        return_value=_resource('semantic-metric', 'metric-1', {'name': 'revenue', 'modelUUID': 'model-1', 'sql': 'SUM(x)'})
    )
