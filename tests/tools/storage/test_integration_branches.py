"""Integration tests for storage tools on a real project with dev branches.

Verifies that bucket/table IDs returned by MCP tools on dev branches
are real, physical Storage API IDs — not stripped/normalized prod_id values
that would break downstream API calls.

Requires environment variables:
    TEST_STORAGE_TOKEN: Storage API token for a project with branched storage OFF
    TEST_STORAGE_API_URL: Storage API URL (e.g. https://connection.keboola.com)

The project MUST have branched storage DISABLED (prefix-based branch model).
This is the scenario where the bug manifested — IDs were being stripped of
their branch prefix, making them unusable for subsequent API calls.

Run with:
    TEST_STORAGE_TOKEN=xxx TEST_STORAGE_API_URL=https://connection.keboola.com \
...
STORAGE_TOKEN = os.environ.get('TEST_STORAGE_TOKEN')
STORAGE_API_URL = os.environ.get('TEST_STORAGE_API_URL')

pytestmark = [
    pytest.mark.skipif(
        not STORAGE_TOKEN or not STORAGE_API_URL,
        reason='TEST_STORAGE_TOKEN and TEST_STORAGE_API_URL required',
    ),
    pytest.mark.asyncio,
]


async def _wait_for_storage_job(client, job: dict, timeout: float = 60.0) -> dict:
    ...
    while asyncio.get_event_loop().time() < deadline:
        ...
        if status in ('success', 'error'):
            if status == 'error':
                raise RuntimeError(f'Storage job {job_id} failed: {result.get("error", {}).get("message", result)}')
            return result
...
async def prod_client():
    """Create a real KeboolaClient pointing at production."""
...
async def dev_branch(prod_client):
    ...
    POST /dev-branches returns a Storage job (202 Accepted).
    We poll the job until it completes, then extract the branch ID from results.
    """
    branch_name = f'integration-{uuid.uuid4().hex[:8]}'

    # POST returns an async job, NOT the branch directly
    job = await prod_client.storage_client.post(
        endpoint='dev-branches',
        data={'name': branch_name, 'description': 'Integration test for MCP PR #464'},
    )
    completed_job = await _wait_for_storage_job(prod_client, job)
    branch_id = str(completed_job['results']['id'])

    yield branch_id

    # Cleanup: delete also returns a job, but we don't need to wait
    try:
...
async def test_bucket(branch_client):
    """Create a bucket on the dev branch, yield its ID, then delete it."""
    bucket_name = f'test-{uuid.uuid4().hex[:8]}'

    result = await branch_client.storage_client.post(
        endpoint='buckets',
        data={'name': bucket_name, 'stage': 'in', 'description': 'MCP integration test'},
    )
    bucket_id = result['id']

    yield bucket_id

    # Cleanup is best-effort; branch deletion also cleans up its buckets
    try:
...
class TestDevBranchBucketIds:
    ...
    async def test_list_all_buckets_returns_real_ids(self, mcp_context, test_bucket):
    ...
    async def test_returned_id_is_api_callable(self, mcp_context, branch_client, test_bucket):
        ...
        # Use the returned ID to call bucket_detail — if stripped, this will 404
        try:
            ...
            pytest.fail(
                ...
                f'This confirms the ID normalization bug.'
            )
...
class TestDevBranchTableIds:
    ...
    @pytest_asyncio.fixture
    async def test_table(self, branch_client, test_bucket):
        """Create a simple table in the test bucket (async — poll job)."""
        ...
        completed_job = await _wait_for_storage_job(branch_client, job)
        table_id = completed_job['results']['id']
        yield table_id
    ...
    async def test_returned_table_id_is_api_callable(self, mcp_context, branch_client, test_table, test_bucket):
        ...
        if not returned_table:
            returned_table = next((t for t in result.tables), None)

        assert returned_table, 'No tables returned'
