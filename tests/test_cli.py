...
class TestLocalLoginFallback:
    ...
    @pytest.mark.asyncio
    async def test_starts_without_a_token_when_no_stored_session(self, monkeypatch) -> None:
        # No credential is never a startup error, on any transport: streamable-http can still be
        # handed one per request via a header (regression: this used to crash the server subprocess
        # before it could even start listening), and both transports start in bootstrap mode, where
        # `create_project` can create a project and a session for it (agent_provisioning RFC).
        monkeypatch.setattr(
